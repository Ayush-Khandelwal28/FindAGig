...
fn add_bid(
    ...
    // Increment the current bidder ID
    state.current_bidder_id += 1;

    let sender = context.sender;

    // Check if the bidder is already registered
    let is_registered = state
        .registered_bidders
        .iter()
        .any(|x| x.address == sender);

    // If the bidder is not registered, register them
    if !is_registered {
        state.registered_bidders.push(RegisteredBidder {
            bidder_id: state.current_bidder_id,
            address: sender,
        });
    }

    // Find the bidder info
    let bidder_info = state
        .registered_bidders
        .iter()
        .find(|x| x.address == sender)
        .expect("Bidder should be registered at this point");
